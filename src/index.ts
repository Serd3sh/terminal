...
export class Terminal {
	...
	private _calcScrollMax() {
		...
		while (vsize < this._size.y && i > 0) {
			vsize += this._parseActualContent(i--).length
		}
		this._scrollMax = Math.max(0, i - Number(vsize > this.size.y) + 1)
	}
	...
	private static _stackGetter(_: Error, stack: NodeJS.CallSite[]) { 
		return stack 
	}
	...
	public static stackTrace() {
		...
		Error.prepareStackTrace = Terminal._stackGetter
		let fakeError = new Error
		Error.captureStackTrace(fakeError)
		
		let stack = Array.from(fakeError.stack as any as NodeJS.CallSite[])	
		let result = stack.map(item => {
			let fName = item.getFileName()?.match(/[a-zA-Z0-9_\- ]+\.[a-zA-Z0-9_\- ]+$/g)
			if (fName == null) return null
			return {
				line: item.getLineNumber(),
				file: fName.pop()!.match(/^[a-zA-Z0-9_\- ]+/g)!.pop(),
	...
	private _split(text: string) {
		...
		while (true) {
			...
			res.push(text.slice(0, sliceEndIndex))
			let m = res[res.length - 1].match(colorSequenceRegex)
			if (m) text = m[m.length - 1] + text.slice(sliceEndIndex)
			else text = text.slice(sliceEndIndex)
		}
		res.push(text)
	...
	private _render() {
		...
		let actualSize = 0
		for (let i = this._scrollOffset; i < this._content.length && actualSize < this._size.y; i++) {
			let parsedLine = this._parseActualContent(i)
			if (i == this._scrollOffset)
				parsedLine.splice(0, this._lineOffset)
			
			for (let j = 0; j < Math.min(parsedLine.length, this._size.y - actualSize); j++) {
				str += `\x1B[${++actualSize};1H${parsedLine[j]}`
			}
		}
		str += `\x1B[${process.stdout.rows};1H> ${' '.repeat(this._size.x-2)}` // clear previous input
		str += `\x1B[${process.stdout.rows};1H> ${this._inputBuffer.join('')}` // apply current input
	...
	public write(...args: any[]) {
		...
		if (autoScrollDown) {
			this._scrollOffset = Math.max(0, this._scrollMax)
			this._lineOffset = this._parseActualContent(this._scrollOffset).length - 1
		}
